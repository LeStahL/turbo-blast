...
"i=min(max(M.x,M.y),0.)+length(max(M,0.));"
"}"
""
...
"m1(q,C)"
""
"\n#define Q(f1 ,r1 ,t1 ,step)void f1(out vec3 o ,in vec3 n ,inout float i ,in vec3 x ,in int h ,out int l ,out vec2 a ){for(l =0;l <h ;++l ){o =n +i *x ;r1(o ,a );if(a .x<1e-4)return ;if(t1 ){l =h ;}i +=step;}}\n"
"Q(V,C,max(max(abs(o.x),abs(o.y)),abs(o.z))>1.1*v,min(a.x,5e-4))"
"Q(p1,C,o.z>.5,min(a.x,1e-2))"
""
